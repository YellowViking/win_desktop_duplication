...
use windows::Win32::Graphics::Dxgi::{
    ...
    DXGI_ERROR_UNSUPPORTED, DXGI_ERROR_WAIT_TIMEOUT, DXGI_OUTDUPL_MOVE_RECT,
};
...
impl DesktopDuplicationApi {
    ...
    pub fn acquire_next_frame_now(&mut self) -> Result<Texture> {
        ...
        if let Some(resource) = self.state.last_resource.as_ref() {
            ...
            let count =
                frame_info.TotalMetadataBufferSize / size_of::<DXGI_OUTDUPL_MOVE_RECT>() as u32;
            let mut move_rects = vec![DXGI_OUTDUPL_MOVE_RECT::default(); count as usize];
            unsafe { dupl.GetFrameMoveRects(buf_size, move_rects.as_mut_ptr(), &mut buf_size) }
                .map_err(|e| {
                    error!("failed to get frame move rects. {:?}", e);
                    DDApiError::Unexpected(format!("failed to get frame move rects. {:?}", e))
                })?;
            move_rects.resize(
                buf_size as usize / size_of::<DXGI_OUTDUPL_MOVE_RECT>(),
                Default::default(),
            );
            ...
            unsafe { dupl.GetFrameDirtyRects(buf_size, dirty_rects.as_mut_ptr(), &mut buf_size) }
                .map_err(|e| {
                error!("failed to get frame dirty rects. {:?}", e);
                DDApiError::Unexpected(format!("failed to get frame dirty rects. {:?}", e))
            })?;
            dirty_rects.resize(
                buf_size as usize / size_of::<Foundation::RECT>(),
                Default::default(),
            );

            self.state.dirty_rects = dirty_rects;
            self.state.moved_rects = move_rects
                .iter()
                .map(|rect| {
                    let src_rect = Foundation::RECT {
                        left: rect.SourcePoint.x,
                        top: rect.SourcePoint.y,
                        right: rect.SourcePoint.x + rect.DestinationRect.right
                            - rect.DestinationRect.left,
                        bottom: rect.SourcePoint.y + rect.DestinationRect.bottom
